...
#         c_bam = self.model[9].cv2.conv.out_channels if hasattr(self.model[9], 'cv2') else 256
#         c_osa = 256  # 一般放在 head 前，用 256 合理
#         self.use_bam = False
        
#         # 根据标志位决定是否插入模块
#         self.bam = BAM(c_bam) if self.use_bam else nn.Identity()
#         # self.bam = BAM(c_bam)
        
#     def _forward_once(self, x, profile=False, visualize=False):
#         # 模型结构大致是 backbone + neck + head
#         for i, m in enumerate(self.model):
#             x = m(x)
#             if i == 9:  # backbone输出层（以yolov8n为例，大约第9层是最后一层C3模块）
#                 x = self.bam(x)  # 插入边界感知模块
#         return x
